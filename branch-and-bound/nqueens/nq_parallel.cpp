...
/**static std::uint64_t count_with_first_row_fixed(int N, int r0)
{
    ...
    // Per-depth arrays (depth i == column i we’re placing).
    std::uint64_t masks[64];       // rows already used
    std::uint64_t left_masks[64];  // rows blocked by left diagonals
    std::uint64_t right_masks[64]; // rows blocked by right diagonals
    std::uint64_t ms[64];          // UNAVAILABLE ∪ TRIED at column i
...
std::uint64_t count_nqueens_parallel(int n)
{
    ...
    {
        //checks what happens when you start with a queen at row i and column 0
        #pragma acc parallel loop reduction(+:total)
        for (int r0 = 0; r0 < n; ++r0) {
            total += count_with_first_row_fixed(n, r0);
        }
